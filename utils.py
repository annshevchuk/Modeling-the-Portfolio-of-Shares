# utils.py
# Общие функции для всех ноутбуков
...
# ------------------------------------------------------------
def get_all_tickers():
    """Загружает список всех акций MOEX (доска TQBR)."""
...
def get_historical_prices(ticker, start_date, end_date):
    """Загружает дневные цены закрытия для одного тикера за период."""
...
def load_prices_dict(pkl_file):
    """Загружает словарь цен из pickle."""
    with open(pkl_file, 'rb') as f:
        return pickle.load(f)


def save_prices_dict(pkl_file, data):
    """Сохраняет словарь цен в pickle."""
...
def annual_return_from_series(prices_series, dividends, trading_days=TRADING_DAYS):
    """Годовая доходность с учётом дивидендов (в процентах)."""
...
def compute_sharpe_with_dividends(prices_series, dividends, rf, trading_days=TRADING_DAYS):
    """Коэффициент Шарпа с учётом дивидендов."""
...
def sortino_with_dividends(prices_series, dividends, rf, trading_days=TRADING_DAYS):
    """Коэффициент Сортино с учётом дивидендов."""
    if prices_series is None or len(prices_series) < 10:
        return None
    returns = prices_series.pct_change().dropna()
    if len(returns) < 5:
...
def equal_weight_metrics(tickers, mean_ret, cov, rf):
    """
    Расчёт годовых доходности, риска и Шарпа для равновзвешенного портфеля.
    """
    n = len(tickers)
    w = np.ones(n) / n
...
# ------------------------------------------------------------
# НОВЫЕ ФУНКЦИИ ДЛЯ ОПТИМИЗАЦИИ С ОГРАНИЧЕНИЯМИ (SHARPE И SORTINO)
# ------------------------------------------------------------

def portfolio_sharpe(weights, returns_df, rf, trading_days=TRADING_DAYS):
    """
    Годовой коэффициент Шарпа для портфеля.
    weights: массив весов (n,)
    returns_df: DataFrame дневных доходностей (T x n), столбцы - тикеры
    rf: годовая безрисковая ставка (десятичная)
    """
...
def portfolio_sortino(weights, returns_df, rf, trading_days=TRADING_DAYS):
    """
    Годовой коэффициент Сортино для портфеля.
    weights: массив весов (n,)
    returns_df: DataFrame дневных доходностей (T x n), столбцы - тикеры
    rf: годовая безрисковая ставка (десятичная)
    """
...
def optimize_portfolio(tickers, mean_ret, cov, returns_df, rf,
    ...
    Оптимизация весов портфеля:
        минимизация дисперсии (риска)
        с ограничениями: sum(w)=1, w_i>=0, Sharpe>=sharpe_min, Sortino>=sortino_min.
    Для n=2 используется аналитическое решение (min variance без ограничений)
        с последующей проверкой ограничений.
    Для n>=3 применяется численная оптимизация SLSQP.
    Критерий улучшения (improved):
        sigma_opt < sigma_eq  И  sharpe_opt >= sharpe_eq * (1 - sharpe_decline_allowed)
    """
    n = len(tickers)
    # Равновзвешенный портфель (для сравнения)
    w_eq = np.ones(n) / n
    ret_eq_annual, risk_eq_annual, sharpe_eq = equal_weight_metrics(tickers, mean_ret, cov, rf)

    # Подготовка матриц и данных
    ...
    if n == 2:
        ...
        if sharpe_opt >= sharpe_min and sortino_opt >= sortino_min:
            # Аналитическое решение допустимо
    ...
    # ---------- n >= 3 : численная оптимизация SLSQP ----------
    constraints = [
    ...
    if not res.success:
        # Оптимизация не удалась – возвращаем равные веса
        w_opt = w_eq
...
def cluster_by_cv(tickers, price_file, max_k=10, random_state=42):
    """Кластеризация акций по коэффициенту вариации (CV=σ/|μ|)."""
